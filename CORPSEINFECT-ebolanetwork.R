...
infection_rate_from_corpse = seq(0,1, by = 0.1) # 
...
x_param = infection_rate_from_corpse # <----------------------------------------- ENTER the Parameter currently tested  
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
for (sim in 1:iterations) {
  ...
  for (iter in seq_along(x_param)) { # Start multiple iterations
    ...
    infection_rate_corpse = infection_rate_from_corpse[iter]
...
full_stats$infection_rate_corpse = as.character(full_stats$infection_rate_corpse)
...
ggplot(data = full_stats, mapping = aes(x = diameter, y = infection_rate_corpse)) +
  geom_jitter(size = .1, aes(color = infection_rate_corpse)) +
...
# Death plot
ggplot(data = life_stats, mapping = aes(x = time, y = deaths, color = infection_rate_corpse)) +
  #geom_jitter(size = 0.01) +
  geom_smooth(data = filter(life_stats, deaths != 0), se = F, size = 1.5) +
  xlab('Timestep') +
  ylab('Average Deaths') +
  labs(color = "Corpse Infection rate") +
  theme_base() +
  #scale_colour_colorblind()
...
# Case plot
ggplot(data = life_stats,mapping = aes(x = time, y = cases, color = infection_rate_corpse)) +
  #geom_jitter(size = 0.01) +
...
# quick stat test on data that is seemingly different
end.times = life_stats$time == 100
end.data = life_stats[end.times,]

CFR_0.1 = end.data[end.data$infection_rate_corpse == 0.1,]
CFR_0.2 =end.data[end.data$infection_rate_corpse == 0.2,]
CFR_0.3 =end.data[end.data$infection_rate_corpse == 0.3,]
CFR_0.4 =end.data[end.data$infection_rate_corpse == 0.4,]
CFR_0.5 =end.data[end.data$infection_rate_corpse == 0.5,]
CFR_0.6 =end.data[end.data$infection_rate_corpse == 0.6,]
CFR_0.7 =end.data[end.data$infection_rate_corpse == 0.7,]
CFR_0.8 =end.data[end.data$infection_rate_corpse == 0.8,]
CFR_0.9 =end.data[end.data$infection_rate_corpse == 0.9,]
CFR_1 =end.data[end.data$infection_rate_corpse == 1,]
