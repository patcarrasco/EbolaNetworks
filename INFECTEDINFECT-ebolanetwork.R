...
infection_rate_from_infected = seq(0,1,by = 0.1)
infection_rate_from_corpse = 0.2 # 
case_fatality_rate = .65  # case fatality # effect on infected --> recovery // Default CFR is .65
amt_doctors_in_network = 0.01 * N # amount of doctors in the network


x_param = infection_rate_from_infected # <----------------------------------------- ENTER the Parameter currently tested
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
full_stats$infection_rate = as.character(full_stats$infection_rate)
...
ggplot(data = full_stats, mapping = aes(x = diameter, y = infection_rate)) +
  geom_jitter(size = .1, aes(color = infection_rate)) +
  xlab('Network Diameter') +
  ylab('Infection Rate from Infected') +
  theme_base() +
...
# Death plot
ggplot(data = life_stats, mapping = aes(x = time, y = deaths, color = infection_rate)) +
  #geom_jitter(size = 0.01) +
  geom_smooth(data = filter(life_stats, deaths != 0), se = F, size = 1.5) +
  xlab('Timestep') +
  ylab('Deaths') +
  labs(color = "Infected Infection rate") +
  theme_base() +
  #scale_colour_colorblind()
...
# Case plot
ggplot(data = life_stats,mapping = aes(x = time, y = cases, color = infection_rate)) +
  #geom_jitter(size = 0.01) +
...
# Quick test for significance

end.times = life_stats$time == 100
end.data = life_stats[end.times,]

# I Change what i'm testing for normality within the brackets of obtained data
CFR_0.1 = end.data[end.data$infection_rate == 0.1,]
CFR_0.2 =end.data[end.data$infection_rate == 0.2,]
CFR_0.3 =end.data[end.data$infection_rate == 0.3,]
CFR_0.4 =end.data[end.data$infection_rate == 0.4,]
CFR_0.5 =end.data[end.data$infection_rate == 0.5,]
CFR_0.6 =end.data[end.data$infection_rate == 0.6,]
CFR_0.7 =end.data[end.data$infection_rate == 0.7,]
CFR_0.8 =end.data[end.data$infection_rate == 0.8,]
CFR_0.9 =end.data[end.data$infection_rate == 0.9,]
CFR_1 =end.data[end.data$infection_rate == 1,]

# Change to $cases/$deaths depending on what's being tested
AVG_CFR0.1 = mean(CFR_0.1$cases)
AVG_CFR0.2 = mean(CFR_0.2$cases)
AVG_CFR0.3 = mean(CFR_0.3$cases)
AVG_CFR0.4 = mean(CFR_0.4$cases)
AVG_CFR0.5 = mean(CFR_0.5$cases)
AVG_CFR0.6 = mean(CFR_0.6$cases)
AVG_CFR0.7 = mean(CFR_0.7$cases)
AVG_CFR0.8 = mean(CFR_0.8$cases)
AVG_CFR0.9 = mean(CFR_0.9$cases)
AVG_CFR1.0 = mean(CFR_1$cases)


# combining it all
case.table = tibble(IR = 0.1, AVG = mean(CFR_0.1$cases))
case.table <- case.table %>%
  add_row(IR = 0.2, AVG = mean(CFR_0.2$cases))
case.table <- case.table %>%         
  add_row(IR = 0.3, AVG = mean(CFR_0.3$cases))
case.table <- case.table %>%
  add_row(IR = 0.4, AVG = mean(CFR_0.4$cases))
case.table <- case.table %>%
  add_row(IR = 0.5, AVG = mean(CFR_0.5$cases))
case.table <- case.table %>%
  add_row(IR = 0.6, AVG = mean(CFR_0.6$cases))
case.table <- case.table %>%
  add_row(IR = 0.7, AVG = mean(CFR_0.7$cases))
case.table <- case.table %>%
  add_row(IR = 0.8, AVG = mean(CFR_0.8$cases))
case.table <- case.table %>%
  add_row(IR = 0.9, AVG = mean(CFR_0.9$cases))
case.table <- case.table %>%
  add_row(IR = 1.0, AVG = mean(CFR_1$cases))


case.table = as.data.frame(case.table)

# perform the ANOVA
my.aov = aov(AVG ~ IR, data = case.table)

shapiro.test(case.table$AVG)
summary(my.aov)# suggests there is significant difference between these values...
