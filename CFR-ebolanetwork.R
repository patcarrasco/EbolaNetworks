...
case_fatality_rate = seq(.25, .95, by = 0.1)  # case fatality # effect on infected --> recovery // Default CFR is .65
amt_doctors_in_network = 0.01 * N # amount of doctors/healthworkers in the network


x_param = case_fatality_rate # <----------------------------------------- ENTER the Parameter currently tested 
#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
full_stats$CFR = as.character(full_stats$CFR)
...
ggplot(data = full_stats, mapping = aes(x = diameter, y = CFR)) +
  geom_jitter(size = .1, aes(color = CFR)) +
...
# Death plot
ggplot(data = life_stats, mapping = aes(x = time, y = deaths, color = CFR)) +
...
# Case plot
ggplot(data = life_stats,mapping = aes(x = time, y = cases, color = CFR)) +
  #geom_jitter(size = 0.01) +
